...
class MoviesViewTest(TestCase):
    ...
    def test_detail_url(self):
        response = self.client.get(reverse("movies"))
        self.assertEqual(response.data[0]["details_url"],
                            reverse("movie_details", kwargs={"id" : "1"}))
    ...
    def test_post_not_superuser(self):
        ...
        response = client.post(reverse("movies"), {"title" : "Star Wars",
                                    "category" : "Fantasy", "rating" : 9.0})
    ...
    def test_patch_not_logged_in(self):
        response = self.client.patch(reverse("movie_details",
                                        kwargs={"id" : "1"}),
    ...
    def test_patch_not_superuser(self):
        ...
        user = User.objects.get(username="simple_user")
        client.force_authenticate(user=user)
        response = client.patch(reverse("movie_details",
                                    kwargs={"id" : "1"}),
                                    {"details" : "best movie ever!"})
        self.assertEqual(response.status_code, 403)
        movie = Movie.objects.get(id=1)
